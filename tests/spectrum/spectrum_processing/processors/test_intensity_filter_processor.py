...
class TestIntensityFilterProcessor(unittest.TestCase):
    def setUp(self):
        ...
        self.min_intensity = 0.10  # Minimum intensity threshold (10% of base peak -> 300/10 = 30)
        self.max_peaks_used = 2  # Maximum number of peaks to retain
        self.validator = SpectrumValidator(min_peaks=1, min_mz_range=50)
        self.processor = IntensityFilterProcessor(
    ...
    def test_change(self):
        ...
        expected_mz = np.array([300.0, 500.0], dtype=float)  # Corresponding to intensities [50.0, 300.0, 150.0]
        expected_intensity = np.array([300.0, 150.0], dtype=float)

        # Validate the results
        np.testing.assert_array_equal(filtered_spectrum.mz, expected_mz, "Filtered mz values do not match.")
        np.testing.assert_array_equal(filtered_spectrum.intensity, expected_intensity,
                                      "Filtered intensity values do not match.")
        self.assertEqual(len(filtered_spectrum.mz), self.max_peaks_used, "Number of peaks retained is incorrect.")
    ...
    def test_change_no_max_peaks(self):
        ...
        np.testing.assert_array_equal(filtered_spectrum.intensity, expected_intensity,
                                      "Filtered intensity values do not match.")
        self.assertEqual(len(filtered_spectrum.mz), len(expected_mz), "Number of peaks retained is incorrect.")

    def test_change_all_peaks_removed(self):
        # Test when min_intensity is set too high to retain any peaks
        self.processor.min_intensity = 1.1  # Set higher than any intensity in the spectrum
        filtered_spectrum = self.processor.process(self.spectrum)

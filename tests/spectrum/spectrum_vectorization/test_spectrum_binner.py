...
class TestSpectrumBinner(unittest.TestCase):
    def setUp(self):
        ...
        self.spectra = [
            sus.MsmsSpectrum(
                ...
                intensity=np.array([100.0, 80.0, 200.0, 300.0, 50.0, 400.0, 20.0], dtype=float),
                retention_time=10.0,
            ),
            sus.MsmsSpectrum(
                ...
                intensity=np.array([120.0, 110.0, 180.0, 310.0, 70.0, 390.0, 25.0], dtype=float),
                retention_time=12.0,
            ),
        ]
    ...
    def test_bin_dimensions(self):
        ...
        self.assertEqual(self.binner.dim, 5, "Binner dimension calculation is incorrect.")
        self.assertEqual(self.binner.min_mz, 3, "Binner minimum mz calculation is incorrect.")
        self.assertEqual(self.binner.max_mz, 10.5, "Binner maximum mz calculation is incorrect.")
    ...
    def test_bin_single_spectrum(self):
        ...
        self.assertEqual(
            binned_matrix.shape, (1, self.binner.dim),
        ...
        expected_intensities = np.array([100.0, 80.0, 200.0, 300.0, 50.0, 400.0, 20.0], dtype=float)
    ...
    def test_bin_multiple_spectra(self):
        ...
        # Verify the shape of the binned matrix
        self.assertEqual(
            binned_matrix.shape, (len(self.spectra), self.binner.dim),
        ...
        expected_intensities_1 = np.array([100.0, 80.0, 200.0, 300.0, 50.0, 400.0, 20.0], dtype=float)

        expected_indices_2 = np.array([0, 1, 2, 2, 3, 4, 4], dtype=int)
        expected_intensities_2 = np.array([120.0, 110.0, 180.0, 310.0, 70.0, 390.0, 25.0], dtype=float)

        # Verify the non-zero entries for each spectrum
        for i, (expected_indices, expected_intensities) in enumerate(
                [(expected_indices_1, expected_intensities_1), (expected_indices_2, expected_intensities_2)]
        ):

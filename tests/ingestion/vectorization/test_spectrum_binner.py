...
class TestSpectrumBinner(unittest.TestCase):
    ...
    def test_bin_single_spectrum(self):
        # Test binning a single ingestion
        ...
        self.assertEqual(
            binned_matrix.shape,
            (1, self.binner.dim),
            "Binned matrix shape is incorrect for a single ingestion.",
        )
        ...
        self.assertTrue(
            ...
            "Binned indices do not match expected values for a single ingestion.",
        )
        self.assertTrue(
            np.array_equal(binned_matrix.data, expected_intensities),
            "Binned intensities do not match expected values for a single ingestion.",
        )
    ...
    def test_bin_multiple_spectra(self):
        ...
        # Expected values for each ingestion
        ...
        # Verify the non-zero entries for each ingestion
        for i, (expected_indices, expected_intensities) in enumerate(
            ...
            self.assertTrue(
                ...
                f"Binned indices for ingestion {i + 1} do not match expected values.",
            )
            self.assertTrue(
                np.array_equal(data, expected_intensities),
                f"Binned intensities for ingestion {i + 1} do not match expected values.",
            )

    def test_empty_spectrum(self):
        # Test binning an empty ingestion
        ...
        self.assertEqual(
            binned_matrix.nnz,
            0,
            "Binned matrix should have no non-zero entries for an empty ingestion.",
        )

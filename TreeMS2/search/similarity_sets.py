...
class SimilaritySets:
    def __init__(self, groups: SpectraDataset):
        self.groups = groups
        self.similarity_sets: npt.NDArray[np.uint64] = np.zeros(
            (self.groups.count_spectra_sets(), self.groups.count_spectra_sets()), dtype=np.uint64
        )
    ...
    @staticmethod
    def load(path: str, groups: SpectraDataset) -> Optional["SimilaritySets"]:
        """

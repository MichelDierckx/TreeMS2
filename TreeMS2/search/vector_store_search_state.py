...
from TreeMS2.compute_distance_matrix_state import ComputeDistancesState
...
class QueryIndexState(State):
    ...
    def _transition(self):
        ...
        if not self.context.contains_states(
            [StateType.CREATE_INDEX, StateType.QUERY_INDEX]
        ):
    ...
    def _generate(self) -> SimilaritySets:
        ...
        hit_histogram_local = HitHistogram(
            bin_edges=construct_bin_edges(self.index.vector_store.vector_count)
        )
        similarity_histogram_local = SimilarityHistogram(
        ...
        for d, i, query_ids in self.index.knn_search(
            ...
            row_indices = np.repeat(query_ids, i.shape[1])[
                flat_mask
            ]  # repeat query id for each neighbour find
            data = np.ones_like(filtered_indices, dtype=bool)  # store 1's for a hit
            similarity_matrix = SimilarityMatrix(
                self.context.groups.total_spectra,
                similarity_threshold=self.similarity_threshold,
            )
            similarity_matrix.update(data=data, rows=row_indices, cols=filtered_indices)

            # post filter (precursor mz window)
            similarity_matrix = post_filtering_pipeline.process(
                similarity_matrix=similarity_matrix,
                total_spectra=self.context.groups.total_spectra,
                target_dir=os.path.join(
                    self.query_results_dir, "filters", f"{batch_nr}_filter.txt"
                ),
            )

            # update similarity sets
            similarity_sets.update_similarity_sets(
                similarity_matrix=similarity_matrix, group_ids=group_ids
            )

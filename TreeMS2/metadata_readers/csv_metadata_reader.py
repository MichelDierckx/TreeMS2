...
class CsvMetaDataReader(MetaDataReader):
    ...
    @classmethod
    def get_metadata(cls, source: str) -> PeakFileGroupMapping:
        """
        Reads metadata for peak files from a CSV or TSV file.

        :param source: str, the path to the CSV or TSV file to read the metadata from.
        ...
        with open(source, mode='r', newline='', encoding='utf-8') as file:
            ...
            # Columns 'PeakFile' and 'Group' must be present
            ...
            # Loop over entries and populate the mapping
            for row in reader:
                peak_file = row['PeakFile']
                group_name = row['Group']
                peak_file_group_mapping.add(peak_file, group_name)  # Add PeakFile to mapping

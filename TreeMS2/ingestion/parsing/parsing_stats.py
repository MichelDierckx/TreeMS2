...
class ParsingStats:
    def __init__(self):
        self.valid = 0
        self.invalid_ids = []
        self.precursor_charge_counts = defaultdict(
            int
        )  # key: charge (int), value: count
...
class ParsingStatsCounts:
    def __init__(
        ...
        valid: int = 0,
        invalid: int = 0,
        precursor_charge_counts: Dict[int, int] = None,
    ):
        self.valid = valid
    ...
    @classmethod
    def from_parsing_stats_list(
        cls, stats_list: List["ParsingStats"]
    ) -> "ParsingStatsCounts":
        total_valid = 0
        total_invalid = 0

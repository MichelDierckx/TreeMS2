...
from TreeMS2.ingestion.spectra_dataset.peak_file.parsing_stats import ParsingStatsCounts
from TreeMS2.ingestion.spectra_dataset.peak_file.peak_file import PeakFile
from TreeMS2.ingestion.spectra_dataset.peak_file.quality_stats import QualityStatsCounts
from TreeMS2.ingestion.spectra_dataset.spectra_set import SpectraSet
...
class SpectraDataset:
    ...
    def get_stats(self) -> Tuple[ParsingStatsCounts, QualityStatsCounts]:
        ...
        for spectra_set in self._spectra_sets:
            ...
            parsing_stats_list = [pf.parsing_stats for pf in spectra_set.get_peak_files() if
                                  pf.parsing_stats is not None]
            quality_stats_list = [pf.quality_stats for pf in spectra_set.get_peak_files() if
                                  pf.quality_stats is not None]
        ...
        aggregated_parsing_stats = ParsingStatsCounts.from_parsing_stats_list(all_parsing_stats)
        # Aggregate quality stats across all peak files in all spectra sets
        aggregated_quality_stats = QualityStatsCounts.from_quality_stats_list(all_quality_stats)
    ...
    @staticmethod
    def _parse_spectra_sets_mapping(path: Path) -> "SpectraDataset":
        """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "spectra_sets": [spectra_set.to_dict() for spectra_set in self._spectra_sets],
        }
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> Optional["SpectraDataset"]:
        try:
            spectra_dataset = cls()
            spectra_dataset.spectra_sets_mapping_path = data["spectra_sets_mapping_path"]
            spectra_dataset._spectra_sets = [SpectraSet.from_dict(spectra_set) for spectra_set in data["spectra_sets"]]
            return spectra_dataset
        except (KeyError, TypeError, AttributeError):
            return None  # Return None if the data structure is incorrect

    @classmethod
    def load(cls, path: str) -> Optional["SpectraDataset"]:

...
class PrecursorMzFilter(MaskFilter):
    def __init__(
        self, precursor_mz_window: float, precursor_mzs: npt.NDArray[np.float32]
    ):
    ...
    def construct_mask(self, similarity_matrix: SimilarityMatrix) -> SpectraMatrix:
        ...
        mask_data = (
            np.abs(precursor_mz_rows - precursor_mz_cols) > self.precursor_mz_window
        )
        ...
        m = csr_matrix(
            (mask_data, (rows, cols)),
            shape=similarity_matrix.matrix.shape,
            dtype=np.bool_,
        )
